...
We loaded and explored the histology imaging-derived data in ML_part1.qmd. Through this process we found:
...
```{r}
data_final <- dat_wide_log %>% select(-donorid)
...
bind_rows(train_cl %>% select(status) %>% mutate(tag='train'),
          test_cl  %>% select(status) %>% mutate(tag='test')
          ) %>% 
  count(tag,status)  %>% 
  ggplot(aes(x = tag, y=n)) + geom_col(aes(fill=status))
