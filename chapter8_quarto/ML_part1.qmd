...
tumour_data <- read_xlsx('data/tumour_data.xlsx')
...
The data contains `r nrow(tumour_data)` tissue samples which have been imaged in a histology lab. Each sample has the anonymized donor id and malignancy status determined by a trained pathologist (benign or malignant).

Automatic quantification of images produces 'feature' metrics including cell shapes, edge (plasma membrane) definition, cytosolic texture, staining intensity, necrosis, vascularity etc.
